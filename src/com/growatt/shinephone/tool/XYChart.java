...
public abstract class XYChart extends AbstractChart {
  ...
  public void draw(Canvas canvas, int x, int y, int width, int height, Paint paint) {
      ...
      for (int j = 0; j < length; j += 2) {
        ...
        if (yValue != MathHelper.NULL_VALUE) {
          ...
          if (points.size() > 0) {
            ...
            float[] p = convertListToFloat(points);
            RectF[] clickableAreasForSubSeries = clickableAreasForPoints(p, yAxisValue, i);
    ...
    boolean showGrid = mRenderer.isShowAxes();
    boolean showCustomTextGrid = mRenderer.isShowCustomTextGrid();
    if (showLabels || showGrid) {
      List<Double> xLabels = getValidLabels(MathHelper.getLabels(minX[0], maxX[0], mRenderer.getXLabels()));
  ...
  /**
   * 把List<Float>转换为float数组
   * @param fList
   * @return
   */
  public float[] convertListToFloat(List<Float> fList){
	  	Float[] ps = new Float[fList.size()];
		for (int i = 0; i < fList.size(); i++) {
			ps[i] = fList.get(i);
		}
		float[] points  = new float[ps.length];
		for(int i=0;i<ps.length;i++){
			points[i]=ps[i].floatValue();
		}
		return points;
  }
  ...
  protected void drawSeries(XYSeries series, Canvas canvas, Paint paint, List<Float> pointsList,
    ...
    float[] points = convertListToFloat(pointsList);
  ...
  protected void drawXLabels(List<Double> xLabels, Double[] xTextLabelLocations, Canvas canvas,
    ...
    boolean showGrid = mRenderer.isShowCustomTextGrid();
