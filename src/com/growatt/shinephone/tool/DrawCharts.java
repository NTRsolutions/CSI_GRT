 ...
 * 
 */
public class DrawCharts extends com.growatt.shinephone.tool.AbstractDemoChart {
	...
	public XYChart execute(final Context context, List<double[]> dataList, String title, String time, int maxTiem, int xLable, int maxY, int nowTime, int color) {
		...
		setChartSettings(renderer, "", "", 4, nowTime + 10, 0,  maxY + maxY / 10, Color.BLACK, Color.BLACK);// 0.5是x坐标显示的最小值，12.5是x坐标显示的最大值
		...
		renderer.setBackgroundColor(Color.parseColor("#ffffff"));// 背景颜色
		renderer.setMarginsColor(Color.parseColor("#ffffff"));// 边框外侧颜色
		renderer.setAxesColor(Color.BLACK);// x轴颜色
		renderer.setZoomEnabled(true);
		renderer.setXLabels(0);// 把数字过滤掉
		renderer.setXLabelsColor(Color.BLACK);  // x轴上刻度颜色
		renderer.setYLabelsColor(0, Color.BLACK);  // y轴上刻度颜色
...
//		renderer.setSelectableBuffer(5); //点击区域的大小 
				...
				for (int i = 0; i < length; i++) {
					...
					seriesRenderer.setFillBelowLineColor(Color.parseColor("#28de6d"));
